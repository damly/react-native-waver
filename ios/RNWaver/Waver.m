...
- (void)setup
{
    ...
    self.waverLevelCallback = ^(Waver * waver) {
        waver.level = waver.value;
    };
}
...
- (void)setColor:(UIColor *)color
{
    ...
    for(int i=0; i < self.numberOfWaves; i++)
    {
        ...
        UIColor *color = [self.waveColor colorWithAlphaComponent:(i == 0 ? 1.0 : 1.0 * multiplier * 0.4)];
        waveline.strokeColor = color.CGColor;
        [waves addObject:waveline];
    }
    
    for(int i=0; i < self.numberOfWaves; i++)
    {
        CAShapeLayer *oldWaveline = [self.waves objectAtIndex:i];
        CAShapeLayer *waveline = [waves objectAtIndex:i];
        [self.layer replaceSublayer: oldWaveline with: waveline];
    }
